...
/** In a FullMatrix, all elements are stored.
 *  \author Droske
 */
template < class _DataType >
...
public:
  ...
  FullMatrix ( const FullMatrix<_DataType>& m, CopyFlag copyFlag = aol::DEEP_COPY ); // FullMatrix is used as return value elsewhere, thus cannot be explicit.
  ...
  FullMatrix<_DataType>& operator*= ( const _DataType alpha );

  FullMatrix<_DataType>& operator*= ( const FullMatrix<_DataType> &mat );

  FullMatrix<_DataType>& operator= ( const Matrix<_DataType> &Mat );
...
#pragma omp parallel for
  ...
  void makeLeftInverse ( FullMatrix<_DataType>& A_1Inv,
                         FullMatrix<_DataType>& A_2,
                         PermutationMatrix<_DataType>& P );

  void makeSubstitutionMatrix ( const Vector<_DataType> &b,
                                Vector<_DataType> &q,
                                FullMatrix<_DataType> &/*F*/,
                                PermutationMatrix<_DataType>& P );
  ...
  void transposeTo ( FullMatrix< _DataType > &Dest ) const  {
...
template < class _DataType >
bool FullMatrix< _DataType >::makeRowEntriesWarningPrinted = false;
...
/**
 * A class for block matrices that does its own memory management.
...
public:
  ...
  SparseBlockMatrix<MatrixType>& operator*= ( const typename MatrixType::DataType alpha ) {
     cout << "DID YOU CALL ME? SparseBlockMatrix" << endl;     
    for ( int i = 0; i < this->getNumRows (); ++i ) {
      for ( int j = 0; j < this->getNumCols (); ++j ) {
        if ( this->getPointer ( i, j ) )
...
/** A class for block matrices based on BlockOpBase which does its own memory management
...
public:
  ...
  */
  BlockMatrix<MatrixType >& operator*= ( const DataType alpha ) {
    cout << "DID YOU CALL ME? BlockMatrix" << endl;  
    for ( typename vector< MatrixType* >::iterator it = this->_blockEntryPointers.begin(); it != this->_blockEntryPointers.end(); ++it ) {
      * ( *it ) *= alpha;
    }
    return *this;
