...
//! Computes the whole energy-functional.
...
class IsotropicEnergy : public aol::Op<aol::Vector<typename ConfiguratorType::RealType>, aol::Scalar<typename ConfiguratorType::RealType> > {
  typedef typename ConfiguratorType::RealType RealType;
  const typename ConfiguratorType::InitType &_grid;
  ...
  const RealType _delta;
public:
  IsotropicEnergy(const typename ConfiguratorType::InitType &Initializer,
