...
public:
  ...
  CSRMatrix& operator= ( const SparseMatrix<DataType> &sparseMatrix ) {
    this->setZero ();
    this->_numRows = sparseMatrix.getNumRows ();
    this->_numCols = sparseMatrix.getNumCols ();
    setFromSparse ( sparseMatrix );
  ...
  //! Return sum of absolute value of row values 
  DataType getRowSumAbs ( const int row ) const {
    DataType val = 0.0;
    for ( int j = this->_indPointer[row]; j < this->_indPointer[row+1]; ++j ) val += aol::Abs(this->_value[j]);    
    return val; 
  }
  
  //! Multiply row by scalar 
  void multiplyRowByScalar ( const int row, const DataType & value ) {
    if ( row >= this->getNumRows() )
      throw aol::Exception ( "CSR_Matrix::multiplyRowByScalar(row,value): row number is greater than matrix size!", __FILE__, __LINE__ );
    for ( int j = this->_indPointer[row]; j < this->_indPointer[row+1]; ++j ) this->_value[j] *= value;    
  }
