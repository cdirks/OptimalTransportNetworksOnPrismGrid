...
#ifdef __APPLE__ // This seems to be the only way to get SuiteSparse_long to work under OS X
...
bool fileNameEndsWith ( const char* FileName, const char* Ending ){
  ...
  return ( ( strlen ( s.c_str() ) > strlen ( Ending ) )
           && ( strcmp ( s.c_str() + strlen ( s.c_str() ) - strlen ( Ending ), Ending ) == 0 ) );
}

bool hasBzipSuffx ( const char *FileName ) {
  return ( fileNameEndsWith ( FileName, ".bz2" ) || fileNameEndsWith ( FileName, ".q2bz" ) );
}
...
//! @defgroup FunctionalAnalysis FunctionalAnalysis
//!

//! @defgroup Operator Operator
//! @ingroup FunctionalAnalysis

//! @defgroup InputOutput InputOutput
//!
...
//! @defgroup qc::Element qc::Element
//!
