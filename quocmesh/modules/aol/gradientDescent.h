...
/**
 * Nearly identical to the class GradientDescent except that it uses
...
//Allow to record the history of evoluation
template <typename ConfiguratorType>
class RecordedGradientDescent
: public GradientDescent<ConfiguratorType, aol::MultiVector<typename ConfiguratorType::RealType> >
{

   typedef typename ConfiguratorType::RealType RealType;

   typedef GradientDescent<ConfiguratorType, aol::MultiVector<typename ConfiguratorType::RealType> >  SuperClass;

    /**Output stream, initialize by constructor*/
    std::ofstream& _os;
 ...
 public:
    RecordedGradientDescent(std::ofstream&  os,
                            int outputID,
                            ...
                            const int MaxIterations = 50,
                            const RealType StartTau = aol::ZOTrait<RealType>::one):
                            SuperClass(Initializer,E, DE, MaxIterations,StartTau),
                            _os(os),
                            _outputID(outputID)
                            {}

    virtual ~RecordedGradientDescent() {}

    /**Recall*/
    virtual void apply( const aol::MultiVector<RealType> &Arg, aol::MultiVector<RealType> &Dest ) const{
       // class super  class's apply
       SuperClass::apply(Arg,Dest);

       // output
          char temString[1000];
          sprintf(temString,"%3d\t%e",_outputID,this->_startTau);
          _os<<temString<<endl;
    }
};
