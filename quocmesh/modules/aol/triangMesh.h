...
public:
  ...
  TriangMesh ( int level ) : _vertex ( 3, ( ( 1 << level ) + 1 ) * ( ( 1 << level ) + 1 ) ) {
      
      int n = ( 1 << level ) + 1;
      int numEls = 1 << ( 2*level + 1 );
      int m = 1 << ( level + 1 );
      DataType step = 1.0/static_cast < DataType > ( n-1 );
      aol::MultiVector < DataType > vertex ( 3, n*n );
      CellArray triang ( numEls );
      aol::Vec < 3, DataType > coords;
      // Set vertex coordinates
      for ( int i = 0; i < n*n; ++i ) {
          coords[0] = static_cast < DataType > ( ( i % n ) ) * step; 
          coords[1] = static_cast < DataType > ( ( i - ( i % n ) ) / n ) * step;
          coords[2] = 0.0;
          vertex.set ( i, coords );
      }
      // Set elements 
      aol::Vector < int > aux ( 1 << ( level+1 ) );
      aux[0] = 1; aux[1] = n;
      for ( int i = 2; i < 1 << ( level+1 ); ++i ) {
          aux[i] = aux[i-2] + 1;
      }
      for ( int i = 0; i < numEls; ++i ) {
          ...
          aol::Vec3 < int > el;
          if ( i % 2 == 0 ) {
              el[0] = r*n + floor((p+1)/2); el[1] = r*n + aux[p]; el[2] = (r+1)*n + floor((p+1)/2); 
          }
          else {
              el[0] = (r+1)*n + floor((p+1)/2); el[1] = r*n + aux[p]; el[2] = r*n + floor((p+1)/2);
          }
          triang[i] = el;
      }
      _vertex = vertex;
      _triang.reset ( new CellArray ( triang ), true );
...
public:
  ...
  void createVertexData ( const vector<string> & vertexDataDescr ) {
    ...
    reallocateVertexData ( vertexDataDescr.size() );
  }
  ...
  void createTriangData ( const vector<string> & triangDataDescr ) {
    ...
    reallocateTriangData ( triangDataDescr.size() );
  }
  ...
  void saveAsPLY ( string filename ) const {
    saveAsStanfordOrUDPLY ( filename, STANFORD_PLY );
  }
  ...
  void loadBasedOnSuffix ( string filename ) {
    if ( fileNameEndsWith ( filename.c_str(), ".ply" ) )
      ...
      throw aol::Exception ( "aol::TriangMesh<DataType,TriangleType>::loadBasedOnSuffix(): unknown suffix", __FILE__, __LINE__ );
  }
...
public:
  ...
  void saveAsStanfordOrUDPLY ( string filename, aol::PLY_FORMAT format, const int Precision = -1 ) const {
...
void saveAsLegacyVTK ( string filename ) const {
  ...
  meshSaver.saveAsLegacyVTK ( filename );
    ...
    //! save as vtu 
void saveAsVTK ( string filename ) const { 
    ...
    out << "<Piece NumberOfPoints=\"" << this->getNumVertices () << "\" NumberOfCells=\"" << this->getNumTriangs () << "\">\n";
    ...
    for ( NodeIteratorType nodeit ( *this ); nodeit.notAtEnd(); ++nodeit ) {
      indices.push_back  ( std::make_pair ( nodeit.getIndex(), this->getVertex ( nodeit.getIndex() ) ) );
      for ( int i = 0; i < 3; ++i ) out << this->getVertex ( nodeit.getIndex() )[i] << ' ';
      out << '\n';
    }
    out << "</DataArray>\n";
    out << "</Points>\n";
    ...
    const unsigned int off = 3;
    // Set offsets for triangular cells written above:
    for ( ElementIteratorType elit ( *this ); elit.notAtEnd(); ++elit ) {
      out << off * counter++ << "\n";
    }
    out << "</DataArray>\n";
    ...
    //! save as vtu 
void saveAsVTK ( string filename, const aol::Vector < RealType > & pointData ) const { 
...
void loadFromStanfordOrUDPLY ( string filename, aol::PLY_FORMAT format ) {
  ...
  while ( file.eof() == false ) {
    ...
    if ( ( ch != 0 ) && ( ch != 9 ) && ( ch != 10 ) && ( ch != 13 ) && ( ch != 32 ) ) {
      throw aol::FileFormatException ( "TriangMesh::loadFromStanfordOrUDPLY(): file contains too much data (reading a Stanford PLY file as UDPLY?).", __FILE__, __LINE__ );
    }
  }
}
...
public:
  ...
  typedef typename TriangMesh < DataType, TriangleType >::CellArray CellArray;  
  ...
  FETriangMesh ( MultiVector<DataType> & vertex, CellArray & triang  ) : TriangMesh< DataType, TriangleType > ( vertex, triang ) {}
  
  FETriangMesh ( int level ) : TriangMesh < DataType, TriangleType > ( level ) {}
