...
public:    
  ...
  typedef typename aol::FETriangMesh<RealType, TriangleType>::CellArray CellArray;
...
public:
  ...
  AdaptiveFETriangMesh ( aol::MultiVector < RealType > & vertex, CellArray & triang ) : aol::FETriangMesh < RealType, TriangleType > ( vertex, triang ) , _markedForRefinement( this->getNumFaces() ) {
    this->makeNeighbour();
    unmarkAll();
  }
  ...
  AdaptiveFETriangMesh ( int level ) : aol::FETriangMesh<RealType, TriangleType > ( level ) {
    _markedForRefinement.reallocate ( this->getNumFaces() );
    this->makeNeighbour();
    unmarkAll();   
  }
