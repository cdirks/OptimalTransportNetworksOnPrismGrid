...
template <class _DataType>
aol::FullMatrix<_DataType>& aol::FullMatrix<_DataType>::operator*= ( const aol::FullMatrix<_DataType> &mat ) {
...
template <class _DataType>
void aol::FullMatrix<_DataType>::printHead ( ostream& os ) const {
  os << "FullMatrix " << this->getNumRows () << " x " << this->getNumCols () << endl;
}

template <class _DataType>
void aol::FullMatrix<_DataType>::readHead ( istream& is ) {
  string s;
  char c;
  int m, n;
...
template <class _DataType>
bool aol::FullMatrix<_DataType>::isSelfTestOk () {
  const int dim = 6;
  int i, j;
...
template <class _DataType>
aol::FullMatrix<_DataType>::FullMatrix ()
    : Matrix<_DataType> ( 0, 0 ) {
  data.reallocate ( 0 );
}

template <class _DataType>
aol::FullMatrix<_DataType>::FullMatrix ( int m, int n )
    : Matrix<_DataType> ( m, n ) {
  data.reallocate ( this->_numRows * this->_numCols );
}

template <class _DataType>
aol::FullMatrix<_DataType>::FullMatrix ( const aol::Vector<_DataType>& V, bool Transpose )
...
template <class _DataType>
aol::FullMatrix<_DataType>::FullMatrix ( const aol::Vector<_DataType>& Vec, const int m, const int n, CopyFlag copyFlag )
...
template <class _DataType>
aol::FullMatrix<_DataType>::FullMatrix ( const aol::Op<aol::Vector<_DataType> >& op, int m, int n )
    : Matrix<_DataType> ( m, n ) {
  data.reallocate ( m * n );
...
template <class _DataType>
aol::FullMatrix<_DataType>::FullMatrix ( const aol::Matrix<_DataType>& m )
    : Matrix<_DataType> ( m ) { 
  data.reallocate ( this->_numRows * this->_numCols );
...
template <class _DataType>
aol::FullMatrix<_DataType>::FullMatrix ( const aol::FullMatrix<_DataType>& m, CopyFlag copyFlag )
    : Matrix<_DataType> ( m ) {
  if( copyFlag != aol::DEEP_COPY )
    throw aol::Exception ( "aol::FullMatrix<_DataType>::FullMatrix: unimplemented copyFlag!", __FILE__, __LINE__ );
...
template <class _DataType>
void aol::FullMatrix<_DataType>::setZero ( ) {
  int i, j;
  for ( i = 0 ; i < this->_numRows ; ++i ) {
    for ( j = 0 ; j < this->_numCols ; ++j ) {
      ref ( i, j ) = 0;
...
template <class _DataType>
aol::FullMatrix<_DataType>& aol::FullMatrix<_DataType>::operator*= ( const _DataType alpha ) {
  int i, j;
  for ( i = 0 ; i < this->_numRows ; ++i ) {
    for ( j = 0 ; j < this->_numCols ; ++j ) {
      ref ( i, j ) *= alpha;
...
template <class _DataType>
aol::FullMatrix<_DataType>& aol::FullMatrix<_DataType>::operator= ( const aol::Matrix<_DataType> &mat ) {
  int i, j;
#ifdef BOUNDS_CHECK
  if ( ( mat.getNumRows() != this->_numRows ) || ( mat.getNumCols() != this->_numCols ) ) {
    cerr << "ERROR in aol::FullMatrix<_DataType>::operator=() : check dimensions.." << endl;
    return *this;
  }
#endif
  for ( i = 0 ; i < this->_numRows ; ++i ) {
...
template <class _DataType>
void aol::FullMatrix<_DataType>::applyAddTranspose ( const Vector<_DataType> &src, Vector<_DataType> &dst ) const {
...
template <class _DataType>
void aol::FullMatrix<_DataType>::makeInverse ( const aol::Matrix<_DataType> &mat ) {
#ifdef BOUNDS_CHECK
  if ( ( mat.getNumRows() != this->_numRows ) || ( mat.getNumCols() != this->_numCols ) ) {
    cerr << "ERROR in aol::FullMatrix<_DataType>::makeInverse : check dimensions.." << endl;
    return;
  }
  if ( this->_numRows != this->_numCols ) {
    cerr << "ERROR in aol::FullMatrix<_DataType>::makeInverse : only rectangular matrices can be inverted!" << endl;
    return;
  }
#endif
  aol::FullMatrix<_DataType> A ( this->_numRows, this->_numCols );
  A = mat;
...
template <class _DataType>
void aol::FullMatrix<_DataType>::makeLeftInverse ( aol::FullMatrix<_DataType>& A_1Inv,
                                                   aol::FullMatrix<_DataType>& A_2,
                                                   aol::PermutationMatrix<_DataType>& P ) {
#ifdef BOUNDS_CHECK
  if ( this->_numRows >= this->_numCols ) {
    cerr << "ERROR in aol::FullMatrix<_DataType>::makeLeftInverse: doesn't make sense for M>=N!\n";
    return;
  }
  if ( ( A_1Inv.getNumRows() != this->_numRows ) || ( A_1Inv.getNumCols() != this->_numRows )
    ...
    cerr << "ERROR in aol::FullMatrix<_DataType>::makeLeftInverse : check dimensions.." << endl;
    return;
  }
#endif
  int i, j;
  ...
  int SwapCol;
  aol::FullMatrix<_DataType> A ( this->_numRows, this->_numCols );
  aol::FullMatrix<_DataType> Swapped ( this->_numRows, this->_numCols );
...
template <class _DataType>
void aol::FullMatrix<_DataType>::makeSubstitutionMatrix ( const aol::Vector<_DataType> &b,
                                                          aol::Vector<_DataType> &q,
                                                          aol::FullMatrix<_DataType> &/*F*/,
                                                          aol::PermutationMatrix<_DataType>& P ) {
  int i;
  aol::FullMatrix<_DataType> A_1Inv ( this->_numRows, this->_numRows );
  aol::FullMatrix<_DataType> A_2 ( this->_numRows, ( this->_numCols - this->_numRows ) );
  aol::FullMatrix<_DataType> Upper ( this->_numRows, this->_numCols - this->_numRows );
...
template <class _DataType>
void aol::FullMatrix<_DataType>::makeLU ( const aol::Matrix<_DataType> &mat, aol::PermutationMatrix<_DataType>& P ) {
#ifdef BOUNDS_CHECK
  if ( mat.getNumRows() != this->_numRows || mat.getNumCols() != this->_numCols || this->_numRows != this->_numCols ) {
    cerr << "ERROR in aol::FullMatrix<_DataType>::makeLU: mat has not the same dimensions as this matrix or is not quadratic! \n";
    return;
  }
  if ( P.getNumRows() != this->_numRows ) {
    cerr << "ERROR in aol::FullMatrix<_DataType>::makeLU: PermutationMatrix P has not the correction dimension!\n";
    return;
  }
#endif
  ( *this ) = mat;
...
template <class _DataType>
void aol::FullMatrix<_DataType>::LUSolve ( aol::Vector<_DataType>& X, const aol::Vector<_DataType> &RHS, const aol::PermutationMatrix<_DataType>& P ) const {
#ifdef BOUNDS_CHECK
...
template <class _DataType>
inline void aol::FullMatrix<_DataType>::shiftRowsUp() {
  // Unused, untested
...
template <class _DataType>
inline void aol::FullMatrix<_DataType>::swapRows ( int R1, int R2 ) {
  for ( int j = 0; j < this->_numCols; ++j ) {
...
template <class _DataType>
inline void aol::FullMatrix<_DataType>::swapColumns ( int C1, int C2 ) {
  _DataType tmp;
  for ( int i = 0; i < this->_numRows; ++i ) {
    tmp = ref ( i, C1 );
    ref ( i, C1 ) = ref ( i, C2 );
...
template <class _DataType>
inline void aol::FullMatrix<_DataType>::multRow ( int R1, _DataType alpha, int StartIndex ) {
...
template <class _DataType>
inline void aol::FullMatrix<_DataType>::addToRow ( int R1, int R2, _DataType alpha, int StartIndex ) {
  for ( int j = StartIndex; j < this->_numCols; ++j ) {
...
template <class _DataType>
void aol::FullMatrix<_DataType>::getBlock ( int r, int c, aol::FullMatrix<_DataType>& block ) const {
  int h = block.getNumRows ();
  int w = block.getNumCols ();
#ifdef BOUNDS_CHECK
  if ( r + h > this->getNumRows () || c + w > this->getNumCols () )
    throw aol::Exception ( "aol::FullMatrix<_DataType>::getBlock: Parameter mismatch: Block does not fit", __FILE__, __LINE__ );
#endif
  for ( int i = 0; i < h; ++i )
    for ( int j = 0; j < w; ++j )
...
template <class _DataType>
void aol::FullMatrix<_DataType>::setBlock ( int r, int c, const aol::Matrix<_DataType>& block ) {
  int h = block.getNumRows ();
  int w = block.getNumCols ();
#ifdef BOUNDS_CHECK
  if ( r + h > this->getNumRows () || c + w > this->getNumCols () )
    throw aol::Exception ( "aol::FullMatrix<_DataType>::setBlock: Parameter mismatch: Block does not fit", __FILE__, __LINE__ );
#endif
  for ( int i = 0; i < h; ++i )
    for ( int j = 0; j < w; ++j )
...
template <class _DataType>
void aol::FullMatrix<_DataType>::getSubColumn ( int r, int c, aol::Vector<_DataType>& subcol ) const {
  int h = subcol.size ();
#ifdef BOUNDS_CHECK
  if ( r + h > this->getNumRows () || c >= this->getNumCols () )
    throw aol::Exception ( "aol::FullMatrix<_DataType>::getSubColumn: Parameter mismatch: Block does not fit", __FILE__, __LINE__ );
#endif
  for ( int i = 0; i < h; ++i )
...
template <class _DataType>
void aol::FullMatrix<_DataType>::setSubColumn ( int r, int c, const aol::Vector<_DataType>& subcol ) {
  int h = subcol.size ();
#ifdef BOUNDS_CHECK
  if ( r + h > this->getNumRows () || c >= this->getNumCols () )
    throw aol::Exception ( "aol::FullMatrix<_DataType>::setSubColumn: Parameter mismatch: Block does not fit", __FILE__, __LINE__ );
#endif
  for ( int i = 0; i < h; ++i )
...
template <class _DataType>
void aol::FullMatrix<_DataType>::addSubColumn ( int r, int c, const aol::Vector<_DataType>& subcol ) {
  int h = subcol.size ();
#ifdef BOUNDS_CHECK
  if ( r + h > this->getNumRows () || c >= this->getNumCols () )
    throw aol::Exception ( "aol::FullMatrix<_DataType>::addSubColumn: Parameter mismatch: Block does not fit", __FILE__, __LINE__ );
#endif
  for ( int i = 0; i < h; ++i )
    ref ( r + i, c ) += subcol.get ( i );
}
...
template <class _DataType>
void aol::FullMatrix<_DataType>::getSubRow ( int r, int c, aol::Vector<_DataType>& subrow ) const {
  int w = subrow.size ();
#ifdef BOUNDS_CHECK
  if ( r >= this->getNumRows () || c + w > this->getNumCols () )
    throw aol::Exception ( "aol::FullMatrix<_DataType>::getSubRow: Parameter mismatch: Block does not fit", __FILE__, __LINE__ );
#endif
  for ( int i = 0; i < w; ++i )
...
template <class _DataType>
void aol::FullMatrix<_DataType>::setSubRow ( int r, int c, const aol::Vector<_DataType>& subrow ) {
  int w = subrow.size ();
#ifdef BOUNDS_CHECK
  if ( r >= this->getNumRows () || c + w > this->getNumCols () )
    throw aol::Exception ( "aol::FullMatrix<_DataType>::setSubRow: Parameter mismatch: Block does not fit", __FILE__, __LINE__ );
#endif
  for ( int i = 0; i < w; ++i )
...
template <class _DataType>
_DataType aol::FullMatrix<_DataType>::getFrobeniusNormSqr() const {
...
template class aol::FullMatrix<int>;
template class aol::FullMatrix<double>;
template class aol::FullMatrix<float>;
template class aol::FullMatrix<long double>;
