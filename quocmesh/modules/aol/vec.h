...
public:
  ...
  /** Copy constuctor
   *  ATTN: This constructor will not be used on call-by-value (and other implicit copies), since it is explicit.
   *  Luckily, it also provents the usage of the automatically generated copy constructor, so that call-by-value
   *  cannot be used for this class.
   */
#if !defined ( __GNUC__ ) || ( GCC_VERSION >= 40200 ) // Unfortunately older versions of GCC don't allow aol::Vector to be used inside an std::vector if the copy constructor is explicit.
  explicit
#endif
...
#pragma ivdep
  ...
  */
  ...
  //! sorts the entries of this vector (in-place sorting using std::make_heap/std::sort_heap)
  void sortValues ( );
  ...
  //! Creates a histogram of the data values with linearly distributed bin boundaries.
  //! \author berkels
  void createHistogramOfValues ( aol::Vector<int> &Histo, const int NumBins ) const {
    Histo.reallocate ( NumBins );

    RealType minVal = getMinValue();
    RealType maxVal = getMaxValue();
  ...
  //! \author mevenkamp
  void createHistogramOfValues ( std::vector<std::pair<RealType, int> > &Histo, const int NumBins ) const {
    ...
    createHistogramOfValues ( histo, NumBins );
  ...
  void crateHistogramOfValues ( aol::Vector<RealType> &BinValues, aol::Vector<int> &BinCounts, const int NumBins ) {
