...
public:
  ...
  static void doLocalAssemblyDirichlet ( const FEOpType * feopPtr, MatType & localMatrix, const IteratorEndType & end_it,
                                         MatrixType &Mat, const aol::BitVector * DirichletMask, bool setDirichletNodes, const ElementMaskType * elMask )  {
...
class FELinOpInterface : public FEOpInterface<_ConfiguratorType, aol::Vector<_RealType> > {
public:
  ...
  explicit FELinOpInterface ( const typename ConfiguratorType::InitType &Grid, OperatorType OpType = ONTHEFLY )
      : FEOpInterface<ConfiguratorType, aol::Vector<RealType> > ( Grid ), _mat ( NULL ), _opType ( OpType ) {}

  explicit FELinOpInterface ( const ConfiguratorType &Config, OperatorType OpType = ONTHEFLY )
      : FEOpInterface<ConfiguratorType, aol::Vector<RealType> > ( Config ), _mat ( NULL ), _opType ( OpType ) {}
...
protected:
  void multiplyOnTheFly ( const Vector<RealType> &Arg, Vector<RealType> &Dest ) const {
    typedef typename ConfiguratorType::ElementIteratorType IteratorType;
...
public:
  ...
  template <typename MatrixType>
  void assembleAddMatrix ( MatrixType &Mat, const aol::BitVector * DirichletMask, bool setDirichletNodes = false, qc::ElementMask<typename ConfiguratorType::InitType, ConfiguratorType, ConfiguratorType::Dim>* elMask = NULL ) const {
    typedef typename ConfiguratorType::ElementIteratorType IteratorType;
    ...
    LocalAssemblyHelper<FELinOpInterface<RealType,ConfiguratorType,Imp,IndexMode>,MatrixType,IndexMode>::doLocalAssemblyDirichlet( this, localMatrix, end_it, Mat, DirichletMask, setDirichletNodes, elMask );
  }
    ...
    if ( setDirichletNodes )
      for ( int argComp = 0; argComp < NumVecCompsArg; ++argComp )
        for ( int destComp = 0; destComp < NumVecCompsDest; ++destComp )
          for ( int i = 0; i < DirichletMask->size(); ++i )
            if ( (*DirichletMask)[i] )
              Mat.getReference( destComp, argComp ).add( i, i, aol::NumberTrait<RealType>::one );
...
/**
 * \brief Provides a Finite-Element operators of the form \f$ \int_\Omega \partial_s \phi_i \partial_t \phi_j dx \f$.
...
/** Operator for mixed derivatives weighted by a scalar.
 *  \author Schwen (MEVIS)
 */
template <typename ConfiguratorType>
...
//! provides an easy interface to scaled mass matrices of the form \f$ \int_\Omega \gamma(x)\phi_i \phi_j\f$, where \f$\gamma\f$
//! is a positive scalar function.
/*!
 * This class works in 2D and 3D and can be endowed with arbitrary quadrature rules.
 * \author Droske
...
//! Class to assemble matrices of the form \f$ \left(\int_\Omega A(x)\phi_i\cdot\phi_j\mathrm{d}x\right)_{ij} \f$,
...
//! General Interface for nonlinear FE-operators depending on x, which are locally assembled.
...
/**
 * Provides an easy interface to operators of the form \f$ \int_\Omega f(x,p(x))[\phi]\varphi_i(p(x))\f$,
...
/**
 * This class represents the weighted mass matrix \f$ \left(\int_\Omega w(x)^2\varphi_i(x)\varphi_j(x)dx\right)_{ij} \f$.
...
/**
 * This class represents a scaled mass operator, assembling the matrix \f$ \left(\int_\Omega \|\nabla w\|^2\psi_i\psi_j\right)_{ij} \f$,
...
/**
 * This class represents the weighted stiffness matrix \f$ \left(\int_\Omega w(x)^2\nabla\varphi_i(x)\cdot\nabla\varphi_j(x)dx\right)_{ij}\f$.
 * Here, the \f$ \varphi \f$ represent the finite element base functions, and the domain \f$ \Omega \f$ is represented by the grid
 * which is passed to the constructor. Furthermore, \f$ w \f$ is passed to the constructor as aol::Vector.
 *
 * Same usage as aol::MassOp.
...
//! Generalization of the class FENonlinIntegrationVectorInterface: the number of components and the size of components in
...
/**
 * Like FENonlinIntegrationVectorInterface but for vector-valued integrands.
 *
...
//! provides an easy interface to matrices of the form \f$ \int_\Omega \phi_i (\nabla \phi_j \cdot v(x)) \f$, where \f$v\f$
...
//! provides an easy interface to matrices of the form \f$ \int_\Omega \phi_i \partial_k\phi_j v(x) \f$, where \f$v\f$
