...
public:
  ...
  void saveTimestepPPM ( const int step,
                         const aol::Vector<RealType> &img,
  ...
  void saveTimestepPPM ( const int outerStep,
                         const int step,
                         const aol::Vector<RealType> &img,
                         const qc::GridStructure &grid,
                         const RealType Center,
    ...
    if  ( checkSaveConditions ( step ) ) {
      if ( grid.getDimOfWorld() == 2 ) {
        ...
        aol::Vector<float> red ( 256 );
        aol::Vector<float> green ( 256 );
        aol::Vector<float> blue ( 256 );
        set_colortrans_256 ( &red, &green, &blue );
        img_scaled_save_ctrscl<RealType> ( tmp, filename.c_str(), Center, ( 1. / Radius ), &red, &green, &blue );
  ...
  void saveTimestepPPMTransformTo01 ( const int step,
                                      const aol::Vector<RealType> &img,
                                      const qc::GridStructure &grid ) const {
    saveTimestepPPMTransformTo01 ( -1, step, img, grid );
  }

  void saveTimestepPPMTransformTo01 ( const int outerStep,
                                      const int step,
                                      const aol::Vector<RealType> &img,
                                      const qc::GridStructure &grid ) const {
    if  ( checkSaveConditions ( step ) ) {
      if ( grid.getDimOfWorld() == 2 ) {
        ...
        aol::Vector<float> blue ( 256 );
        set_colortrans_256 ( &red, &green, &blue );
        img_scaled_save<RealType> ( tmp, filename.c_str(), &red, &green, &blue );

        if ( !_quiet )
          cerr << endl << aol::color::reset;
      } else {
        cerr << aol::color::red<< "\n\nWARNING: saveTimestepPPMTransformTo01 not implemented yet in 3D!!!\n\n" << aol::color::reset;
      }
    }
  }
...
public:
  ...
  void initFromParser ( const aol::ParameterParser &Parser, const bool OnlyGetDirectory = false, const char *SaveDirectorySuffix = NULL ) {
    if ( Parser.hasVariable ( "saveDirectory" ) ) {
      string saveDir = Parser.getString( "saveDirectory" );
