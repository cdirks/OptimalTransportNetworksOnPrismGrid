...
/**
* a direct solver for matrices which are symmetric and positive definite
* to compile this class the external cholmod is necessary
*
* \author Wirth
*/
...
/**
* A direct solver for block matrices \f$ B \f$ without zero eigenvalue. The matrix neither needs to be symmetric nor positive definite.
* When calling apply(r,x) the normal equations \f$ B^TB x = B^T r \f$ are solved for \f$ x \f$ by means of aol::CholeskyBlockInverseOp<>.
* To compile this class the external cholmod is necessary.
* Each time the matrix changes one has to create a new instance of this solver.
*
* \author Teusner
*/
...
class CholeskyBiBlockInverseOp :
  public aol::InverseOp<aol::MultiVector<typename ConfiguratorType::RealType> > {
...
* (see quickstart documentation, https://www.cise.ufl.edu/research/sparse/umfpack/UMFPACK/Doc/QuickStart.pdf)
* (a more detailed documentation, https://www.cise.ufl.edu/research/sparse/umfpack/UMFPACK/Doc/UserGuide.pdf)
*
...
private:
  ...
  SuiteSparse_long _n;
  aol::Vector<SuiteSparse_long> _Ap, _Ai;
...
public:
  ...
  inline void setMatrix ( const aol::BlockOpBase<RealType,MatrixType> &BlockMat ) {
      ...
      for ( int localRow = 0; localRow < BlockMat.getReference( row, 0 ).getNumRows(); localRow++ ) {
        ...
        for ( int col = 0; col < BlockMat.getNumCols(); col++ ) {
          ...
          for ( typename vector<typename aol::Row<RealType>::RowEntry >::iterator it = matRow.begin(); it != matRow.end(); ++it ) {
            ...
            SuiteSparse_long totalCol = colOffset + static_cast<SuiteSparse_long>(it->col);
    ...
    _n = static_cast<SuiteSparse_long>( aol::Max( rowInd.getMaxValue(), colInd.getMaxValue() ) + 1 );
    _Ap.resize( _n + 1 );
    _Ai.resize( static_cast<SuiteSparse_long>(val.size()) );
    _Ax.resize( static_cast<SuiteSparse_long>(val.size()) );
...
/**
* a direct QR-solver for square matrices which can be umsymmetric and any kind
* 
...
public:
  CholeskyBlockInverseOp( const int /*MaxNumEntriesPerBlockRow*/ ) { }

  void setMatrix ( const aol::BlockOpBase<RealType,MatrixType> &/*BlockMat*/ ) {
