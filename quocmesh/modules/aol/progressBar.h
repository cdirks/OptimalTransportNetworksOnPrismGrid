...
public:
   ...
   *  Every delta steps of the internal counter an updated output is made.
   */
  void start ( const TYPE maxValue, const int delta = 5, const TYPE incr = 1 ) {
    _value = 0;
    _maxValue = maxValue;
    _delta = delta;
    _incr = incr;
 ...
 *  \author Schwen (MEVIS)
 */
class EstimatingProgressBar : protected ProgressBar<> {
protected:
  double _startedAt;
...
public:
  ...
  void start ( const int maxValue, const int delta = 1 ) {
