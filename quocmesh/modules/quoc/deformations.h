...
/**
 * This class represents the weighted mass matrix \f$ \left(\int_\Omega w(\phi(x))\varphi_i(x)\varphi_j(x)dx\right)_{ij} \f$.
...
/**
 * This class represents the weighted mass matrix \f$ \left(\int_\Omega w(\phi(x))^2\varphi_i(x)\varphi_j(x)dx\right)_{ij} \f$.
 * Here, the \f$ \varphi \f$ represent the finite element base functions, and the domain \f$ \Omega \f$ is represented by the grid
 * which is passed to the constructor. Furthermore, \f$ w \f$ and \f$ d \f$ are passed to the constructor as aol::Vector and
 * aol::MultiVector respectively, where \f$ d \f$ is the displacement such that the deformation \f$ \phi \f$ is given by \f$ d \f$+identity.
 *
...
/**
 * This class represents the weighted mass matrix \f$ \left(\int_\Omega w(\phi^{-1}(x))\varphi_i(x)\varphi_j(x)|det(\nabla\phi^{-1}(x))|dx\right)_{ij} \f$.
 * Here, the \f$ \varphi \f$ represent the finite element base functions, and the domain \f$ \Omega \f$ is represented by the grid
 * which is passed to the constructor. Furthermore, \f$ w \f$ and \f$ d \f$ are passed to the constructor as aol::Vector and
 * aol::MultiVector respectively, where \f$ d \f$ is the displacement such that the deformation \f$ \phi \f$ is given by \f$ d \f$+identity.
 *
...
/**
 * This class represents the weighted mass matrix \f$ \left(\int_\Omega w(\phi^{-1}(x))^2\varphi_i(x)\varphi_j(x)|det(\nabla\phi^{-1}(x))|dx\right)_{ij} \f$.
 * Here, the \f$ \varphi \f$ represent the finite element base functions, and the domain \f$ \Omega \f$ is represented by the grid
 * which is passed to the constructor. Furthermore, \f$ w \f$ and \f$ d \f$ are passed to the constructor as aol::Vector and
 * aol::MultiVector respectively, where \f$ d \f$ is the displacement such that the deformation \f$ \phi \f$ is given by \f$ d \f$+identity.
 *
...
public:
  explicit SquaredWeightDeformMassOp ( const typename ConfiguratorType::InitType &Grid, const aol::MultiVector<RealType> &Displacement, const aol::Vector<RealType> &Weight, aol::OperatorType OpType = aol::ONTHEFLY ) :
    FELinDeformScalarWeightedMassInterface<ConfiguratorType, SquaredWeightDeformMassOp<ConfiguratorType> > ( Grid, Displacement, OpType ),
    _weight( Grid, Weight ) {}
