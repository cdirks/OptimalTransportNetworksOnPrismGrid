 ...
 * \author Berkels
 */
template <typename RealType, int _Dim>
class DefaultArraySaver : public aol::StepSaverBase<RealType, typename qc::ScalarArrayTrait<RealType, _Dim>::ArrayType > {
...
protected:
  void doSaveStep ( const qc::ScalarArray<RealType, Dim> &SaveInput, const int Iteration, const char *OverrideBaseSaveName ) const {
    ...
    if ( ( Dim == qc::QC_2D ) && ( _dontSavePNGIn2D == false ) ) {
      qc::ScalarArray<RealType, Dim> temp ( SaveInput, aol::FLAT_COPY );
