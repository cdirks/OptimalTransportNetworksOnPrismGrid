...
class ColorWheel {
  ...
  aol::Vector<float> _red;
  aol::Vector<float> _green;
  aol::Vector<float> _blue;
public:
  ColorWheel ( const qc::ScalarArray<RealType, qc::QC_2D> &AngleArray, const RealType LowerAngle, const RealType UpperAngle, const bool Rescale )
      ...
      _green ( 256 ),
      _blue ( 256 ) {
    set_morecolortrans_256 ( &_red, &_green, &_blue );
  ...
  void convertScalarToColor ( const qc::ScalarArray<RealType, qc::QC_2D> &Scalar, qc::MultiArray<unsigned char, 2, 3> &BufArray, const RealType a1, const RealType a2 ) {
    ...
    for ( int y = 0; y < numY; y++ ) {
      for ( int x = 0; x < numX; x++ ) {
        ...
        BufArray[0][y*numX+x] = static_cast<unsigned char> ( _red[ static_cast<int> ( b * 255 ) ] * 255 );
        BufArray[1][y*numX+x] = static_cast<unsigned char> ( _green[ static_cast<int> ( b * 255 ) ] * 255 );
        BufArray[2][y*numX+x] = static_cast<unsigned char> ( _blue[ static_cast<int> ( b * 255 ) ] * 255 );
      }
    }
  }
};
    ...
    for ( int y = cy - r;y <= cy + r;y++, i++ ) {
      ...
      if ( n > 1.0e-6 ) {
        ...
        if ( b < a1 || b > a2 ) {
          ...
          // ColorLookupTable::hsv2rgb(hsv,rgb);
          rgb[0] = _red[ static_cast<int> ( a * 255 ) ];
          rgb[1] = _green[ static_cast<int> ( a * 255 ) ];
          rgb[2] = _blue[ static_cast<int> ( a * 255 ) ];
        }
      } else {
        rgb[0] = rgb[1] = rgb[2] = 0;
        continue;
...
public:
  ...
  void saveColoredWheel ( const std::string filename ) {
    qc::MultiArray<unsigned char, 2, 3> bufArray ( _numX, _numY );
 ...
 */
template <typename RealType>
void writeColorField ( const qc::Array<RealType> &vx, const qc::Array<RealType> &vy, const string &filename, RealType factor = -1 ) {
  ...
  wheel.saveColoredWheel ( filename.c_str() );
}
