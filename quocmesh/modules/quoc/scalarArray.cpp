...
void qc::ScalarArray<_DataType, qc::QC_2D>::load ( const char *fileName ) {
  ...
  if ( aol::fileNameEndsWith ( fileName, ".png" ) ) {
    ...
    loadDM3 ( fileName );
  } else if ( aol::fileNameEndsWith ( fileName, ".tif" ) ) {
    loadTIFF ( fileName );
  } else {
  ...
  if ( tif ) {
    ...
    for ( uint32 row = 0; row < height; ++row ) {
      ...
      for ( uint32 col = 0; col < width; ++col ) {
        if ( bits == 16 )
          this->set ( col, row, static_cast<uint16*>(buf)[col] );
        else if ( bits == 8 )
          this->set ( col, row, static_cast<uint8*>(buf)[col] );
        else
          throw aol::UnimplementedCodeException ( aol::strprintf ( "%d bits per sample unimplemented.", bits ).c_str(), __FILE__, __LINE__ );
      }
    }
    _TIFFfree ( buf );
    TIFFClose ( tif );
...
#else
  aol::doNothingWithArgumentToPreventUnusedParameterWarning ( fileName );
  throw aol::Exception ( "Reading TIFF files requires the libtiff or external cimg.", __FILE__, __LINE__ );
#endif
}
