...
public:
  ...
  RealType evaluateIntegrand ( const aol::auto_container<ConfiguratorType::Dim, aol::DiscreteFunctionDefault<ConfiguratorType> > &discrFuncs,
    ...
    if ( !qc::transformCoord<ConfiguratorType> ( this->_initializer, El, RefCoord, offset, transformed_el, transformed_local_coord ) ) {
      return 0;
    }
...
/**
 * \brief Computes \f$ \left(\int (D\psi)(\phi(x))^T(\psi(\phi(x))-x)^2\cdot\varphi_i(x) \mathrm{d}x\right)_i \f$, where arg = \f$ \phi \f$ and
 *        \f$ \psi \f$ is supplied in the constructor.
 *
 * \author Han, Berkels
 */
