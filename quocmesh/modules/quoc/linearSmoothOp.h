 ...
 */
template <typename ConfiguratorType>
class CGBasedInverseH1Metric : public aol::BiOp< aol::Vector<typename ConfiguratorType::RealType> > {
  typedef typename ConfiguratorType::RealType RealType;
public:
  ...
  void apply ( const aol::Vector<RealType> &Arg, aol::Vector<RealType> &Dest ) const {
    if ( Arg.size() != _grid.getNumberOfNodes() )
    ...
    aol::CGInverse<aol::Vector<RealType> > inv ( _systemMat, 1e-8 );
