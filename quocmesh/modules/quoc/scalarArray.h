...
public:
   ...
   *  @throws qcTypeException if the magic number of the file is unknown, or the dimensions of the array contained in that file do not match this one
   *  @throws qcIOException if an io error occured
   ...
   *  @throws qcFileException if the file could not be opened
   *  \author Preusser, Droske, Teusner
   */
   ...
   *  @param comment The comment to be written into the header of the file. If comment is NULL the date and the data type will be written as comment
   *  @throws qcTypeException if the given type is incompatible
   *  @throws qcIOException if an io error occured
   *  @throws qcFileException if the file could not be opened
   *  @throws qcPipeException if the compression pipe could not be opened
   *  \author Preusser, Droske, Teusner
   *  Default is now no compression, since load cannot automatically decompress at the moment
   */
...
public:
   ...
   */
  RealType interpolate ( const aol::Vec2<RealType> &pt ) const {
    return interpolate ( pt[0], pt[1] );
  }

  RealType interpolate_on01 ( const aol::Vec2<RealType> & coord ) const {
    return interpolate_on01 ( coord[0], coord[1] );
  }

  RealType interpolate_on01_periodic ( const aol::Vec2<RealType> & coord ) const {
  ...
  template <class T>
  void cellGradient ( const Element &el, aol::Vec2<T> &Grad ) const {
    Grad[ 0 ] = dx ( el );
    Grad[ 1 ] = dy ( el );
  }

  template <class T>
  void cellGradient ( const Element &el, RealType LocalX, RealType LocalY, aol::Vec2<T> &Grad ) const {
    Grad[ 0 ] = dx ( el, LocalX, LocalY );
    Grad[ 1 ] = dy ( el, LocalX, LocalY );
  }

  template <typename T>
  void gradient ( const aol::Vec2<T> &pos, aol::Vec2<T> &grad ) const {
  ...
  template <typename T>
  void gradientFD ( const aol::Vec2<int> &pos, aol::Vec2<T> &grad ) const {
  ...
  inline RealType dx ( const aol::Vec2<RealType> &Pt ) const {
    return dx ( Pt[0], Pt[1] );
  }

  inline RealType dy ( const aol::Vec2<RealType> &Pt ) const {
    return dy ( Pt[0], Pt[1] );
  }
  ...
  void set ( const aol::Vec2<short> &Coord, DataType V ) {
    Array<DataType>::set ( Coord, V );
  }
  ...
  DataType &getReference ( const aol::Vec2<short> &Coords ) {
    return Array<DataType>::getReference ( Coords );
  }
  ...
  void padFrom ( const ScalarArray<DataType, qc::QC_2D> &image, const DataType FillValue = 0 ) {
    if ( this->getNumX() < image.getNumX() || this->getNumY() < image.getNumY() ) {
      throw aol::Exception ( "ScalarArray<QC_2D>::padFrom(): Image should fit into the current array!\n", __FILE__, __LINE__ );
    }

    const int padX = ( this->getNumX() - image.getNumX() ) / 2;
    const int padY = ( this->getNumY() - image.getNumY() ) / 2;
    ...
    const int tRanX[2] = { aol::Max ( aol::NumberTrait<int>::zero, padX ),
        this->getNumX() - aol::Max ( aol::NumberTrait<int>::zero, padX + ( this->getNumX() - image.getNumX() ) % 2 ) };
    const int tRanY[2] = { aol::Max ( aol::NumberTrait<int>::zero, padY ),
        this->getNumY() - aol::Max ( aol::NumberTrait<int>::zero, padY + ( this-> getNumY() - image.getNumY() ) % 2 ) };
    const int sRanX[2] = { -aol::Min ( aol::NumberTrait<int>::zero, padX + ( this->getNumX() - image.getNumX() ) % 2 ),
        image.getNumX() + aol::Min ( aol::NumberTrait<int>::zero, padX ) };
    const int sRanY[2] = { -aol::Min ( aol::NumberTrait<int>::zero, padY + ( this-> getNumY() - image.getNumY() ) % 2 ),
...
public:
  ...
  */
   ...
   *  @throws qcFileException if one of the files could not be opened
   *  \author Droske
   */
