...
void testNAN( aol::Vector<double> &Arg, char *vecName )
{
  cerr<<blue<<"\ntesting vector "<<green<<vecName<<blue<<"..."<<reset;
...
int main( int argc, char **argv ) {
  ...
  try {
    ...
    bool accelerateGrowth = parser.getInt("accelerateGrowth");    // has to be 0 or 1
    ...
    for ( elit = grid.begin(); elit != grid.end(); ++elit )
      if ( Radii.get( (*elit)[0], (*elit)[1], (*elit)[2] ) > 0 ) { narrowGrid.insert( *elit ); count++; }

    // define the boundary of the narrow band.
    narrowGrid.extractEdges();

    double percent = count * 100. / static_cast<double>(Radii.size());
    cerr << red << "Computing on "<<count<<" nodes, i.e. on "<<percent<<" percent.\n";
    ...
    // the anisotropy
    qc::RotatedEllipsoidAnisotropy<double> ellipsoid(  parser.getDouble("aHalfAxis"), 1., 1.,   eps);
    cerr << "The half-axis of the ellipsoid are "<<parser.getDouble("aHalfAxis")<<",1,1, epsilon is "<<eps<<".\n\n";

    // integrate over gamma_z( grad u )
    qc::LocalRotatedAnisotropyIntegrationOp< ConfigType, qc::RotatedEllipsoidAnisotropy<double> >
      mcmAnisoInt( narrowGrid, ellipsoid, &rotationX, &rotationY, &rotationZ );

    // border-integral of the narrowband
    nb::IntegrateAnisotropyOverBoundary< double, ConfigType, NarrowBandGridType,
                                         qc::RotatedEllipsoidAnisotropy<double>, qc::QC_3D >
      nbAnisoBorderInt( config, narrowGrid, eps, ellipsoid );

    qc::LocalRotatedAnisotropyStiffOp< ConfigType, qc::RotatedEllipsoidAnisotropy<double> >
       mcmAnisoStiff( narrowGrid, aol::ASSEMBLED, img, ellipsoid, eps,
...
//       testNAN(rhs, "Rhs nach rotatedIntOp...");
      ...
      for (int i=0; i<N; i++) {
        ...
        for (int j=0; j<N; j++) {
          ...
          for (int k=0; k<N; k++) {
            ...
            // have a radius > 0 and a direction that is not 0
...
//                 cerr<<aol::color::red;
                ...
                if (norm != 0) {
                  ...
                  double correctionTerm = locCurvVal / ballMass;
                  if ( accelerateGrowth ) correctionTerm *= (1. - abs( n*rotDirection ));
      ...
      watchBall.stop();
      cerr << "\rBerechnung lokaler Kruemmungen abgeschlossen " << aol::color::black;
...
//       testNAN(rhs, "Rhs nach nbAnisoBorderInt...");
