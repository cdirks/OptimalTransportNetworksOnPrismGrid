...
//             else cerr<<".";
            ...
            if (radius > 0)
            {
              ballMass = massIntegrator.integrateBall(radius*radiusFactor*grid.H(), coords);
              if (ballMass != 0)
              {
                locCurvVal = curvatureIntegrator.integrateBall(radius*radiusFactor*grid.H(), coords);
                ...
                rotDirection /= rotDirection.norm();
      ...
      watchBall.stop();
      cerr << aol::color::black << "Berechnung lokaler Massen abgeschlossen (" << watchBall.elapsedCpuTime() << "s.)\n";
