...
/* *******************************************************************
 * Coupled advection-diffusion problem with constant velocities
 * ******************************************************************* */
...
void compute_aroot_inflow_values ( aol::Vector<double> &Rp_in, double inflow_val, const bool inflow_pulse = false ) {
  ...
  if ( inflow_pulse )
    if ( Rp_in.size() > 200 )
      for ( int t = 0; t < 200; ++t )
        Rp_in[t] += 0.5 * ( 1 - cos ( 2.0 * aol::NumberTrait<double>::pi * t / 200.0 ) );

  // sin( 2.0 * PI * t / 200.0 ); // 0.5 * ( 1.0 + sin( 2.0 * aol::NumberTrait<double>::pi * t / 200.0 ) );
...
int main ( int argc, char** argv ) {
  if ( argc == 2 ) {
    try {
      ...
      cerr << "tau = " << tau << ", 1.0 / tau = " << ( 1.0 / tau ) << endl;
      ...
      const aol::colorTrans colTrans = aol::HSV_BLUE_TO_RED;
      ...
      for ( int t = 1; t <= timesteps; t++ ) {
        ...
        //A_brhsmv.clear();    A_dwarfmv.clear();     V_brhsmv.clear();    V_dwarfmv.clear();    A_mutsrc.clear();    V_mutsrc.clear();
        ...
        //          tvgeom.add_atree_outflow_rhs( A_brhsmv, A_mutsrc, A_M_block, A_Me_block, tau );
        //          tvgeom.add_vtree_inflow_rhs(  V_brhsmv, V_mutsrc, V_M_block, V_Me_block, tau );
        ...
        //        tvgeom.add_ves_tis_transfer_rhs( A_brhsmv, A_mutsrc, A_M_block, A_Me_block, V_brhsmv, V_mutsrc, V_M_block, V_Me_block, tau, params.getDouble("perc") );

        // compute and set up corresponding source terms for the tissue
        //        tvgeom.add_corresponding_tissue_sources( A_mutsrc, V_mutsrc, tis_rhs, C_ves / C_tis);
        ...
        //        set_probe_influence( params, tvgeom.tissue_data );
        ...
        if ( ! ( t % save_every ) ) { // only save every save_every timesteps
          ...
          if ( !static_cast<bool> ( params.getInt ( "topview" ) ) ) {
            tvgeom.save_advect_step ( profilefn, colTrans, zzoom, color_scale, rot, shift, true );
          } else {
            zzoom *= 1.0;
