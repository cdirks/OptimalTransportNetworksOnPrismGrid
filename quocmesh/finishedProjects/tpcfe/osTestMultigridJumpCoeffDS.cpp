...
int main ( int argc, char** argv ) {
  try {
    ...
    aol::ParameterParser params ( argc < 2 ? "par/osTestMultigridJumpCoeffDS.par" : argv[1] );
    cerr << "Reading from " << ( argc < 2 ? "par/osTestMultigridJumpCoeffDS.par" : argv[1] ) << endl;
    ...
    for ( short s = 0; s < dcMinusSteps; ++s ) {
      ...
      {
        char filename[1024];
        params.getString ( "levelsetFilename", filename );
        levelset.load ( filename );
      ...
      if ( params.getInt ( "doCG" ) == 1 ) {
        timer.start();
        aol::CGInverse< aol::Vector<RealType> > cgsolver ( sysMat, eps, 10000 );
