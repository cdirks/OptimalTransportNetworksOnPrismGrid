...
int main( int argc, char **argv ) {
  ...
  try {
    aol::ParameterParser parser( argc, argv, "ParameterFiles/flowseg.par" );

    aol::StopWatch watch;
    watch.start();

    VectorFieldSegmentation<ConfType, HeavisideFuncType, 1> vectorFieldSegmentation( parser );
    vectorFieldSegmentation.computeTimesteps();
