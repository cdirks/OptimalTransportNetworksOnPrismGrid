...
try 
{
	...
	if( Parser.hasVariable( "UInitFile" ))
	{
		Parser.getString( "UInitFile", UInitFile );
	}
	if( Parser.hasVariable( "WInitFile" ))
	{
		Parser.getString( "WInitFile", WInitFile );
	}
	...
	//DispU_tmp.setAll(0.);
	printf("%s",UInitFile);
	DispU.load( UInitFile );
	DispU_tmp.load( UInitFile );

	// initialize u
	//qc::ScalarArray<RType, qc::QC_1D> DispURaw( Grid2D);
	//DispURaw.load( UInitFile );
	//aol::MultiVector<RType> DispUMV( 3, 129*129 );
	//aol::MultiVector<RType> DispU_tmpMV( 3, 129*129 );
	//printf("Load U.");
	//DispUMV.load( UInitFile );
	//DispU_tmpMV.load( UInitFile );
	//qc::MultiArray<RType, 2, 3> DispU(DispUMV);
	//qc::MultiArray<RType, 2, 3> DispU_tmp(DispU_tmpMV);
	//printf("Loaded U.");
	...
	const aol::DiscreteFunctionDefault<ConfType3D> SDFDiscFuncs(Grid3D, ReferenceDataSDF);
	Projector<ConfType3D> Proj( Grid3D, SDFDiscFuncs );
	const ConfType3D Configurator3D(Grid3D);
	for( unsigned int i = 0; i < TemplateDataPoints.size(); i++ )
	{
		aol::Vec3<RType> y_i;
		Y.getTo( i, y_i );
		...
		if ( Configurator3D.getLocalCoords( y_i, TransformedEl, TransformedCoord ) )
		{
			DispW.set( i, Proj.projectOnG( y_i, TransformedEl, TransformedCoord )-y_i);
		}
	}
	...
	aol::NewtonInfo<RType> newtonInfo ( 1E-10, 2000, 1E-20, 1000, aol::STOPPING_ABSOLUTE );
	...
	clock_t solver_stop = clock();
	std::cout << "solver runtime: " << ((solver_stop - solver_start)*1000.)/((float) CLOCKS_PER_SEC) << std::endl;
	std::cout << "done." << std::endl;
#endif 	
	...
	char DispUFile[1024];
	sprintf ( DispUFile, "%s/u_k_%f_l_%f_m_%f_it_%i_%f_%%i.pgm", DispU_Dir, Kappa, Lambda, Mu, SolverIterations, Eps );
	DispU.save( DispUFile, qc::PGM_DOUBLE_BINARY );
	...
	char DispWFile[1024];
	sprintf ( DispWFile, "%s/w_k_%f_l_%f_m_%f_it_%i_%f.pgm", DispW_Dir, Kappa, Lambda, Mu, SolverIterations, Eps );
	DispW.save( DispWFile );
	...
	for( int x = 0; x < GridSize2DX; x++ )
	{
		for( int y = 0; y < GridSize2DY; y++ )
		{
			...
			//Vec3D phiGi; DispU.getTo( i, phiGi );
			//phiGi += Gi;
