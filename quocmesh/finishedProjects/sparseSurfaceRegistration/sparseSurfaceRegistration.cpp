...
try 
{
	...
	aol::MultiVector<RType> S( ConfType3D::Dim, TemplateDataPoints.size() );	
	aol::MultiVector<RType> Y( ConfType3D::Dim, TemplateDataPoints.size() );	
	for( unsigned int i = 0; i < TemplateDataPoints.size(); i++ )
	{
		S[0][i] = TemplateDataPoints[i].X;
		S[1][i] = TemplateDataPoints[i].Y;
		S[2][i] = TemplateDataPoints[i].Z;
	...
	std::cout << TemplateDataPoints.size() << std::endl;
	...
	//RType MaxMagnitude = 0.;
	//for( int x = 0; x < Grid3D.getNumX(); x++ )
	//{
	//	for( int y = 0; y < Grid3D.getNumY(); y++ )
	//	{
	//		RType DefX = SyntheticDeformationC0.get(x,y,0);
	//		RType DefY = SyntheticDeformationC1.get(x,y,0);
	//		RType Magnitude = sqrt(DefX*DefX + DefY*DefY);
	...
	gradientDescent_solver.setConfigurationFlags ( GDComponentWiseType::USE_NONLINEAR_CG | GDComponentWiseType::USE_GRADIENT_BASED_STOPPING | ( NoConsoleOutput ? GDComponentWiseType::DO_NOT_WRITE_CONSOLE_OUTPUT : 0 ) );	
	...
	std::cout << "solver runtime: " << (solverStop - solverStart) << std::endl;
	std::cout << "done." << std::endl;
#else
	//QuasiNewton
	...
	std::cout << "solver runtime: " << ((solver_stop - solver_start)*1000)CLOCKS_PER_SEC << std::endl;
	std::cout << "done." << std::endl;
#endif
