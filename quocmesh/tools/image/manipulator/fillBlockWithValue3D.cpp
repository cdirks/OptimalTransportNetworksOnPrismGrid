...
/** \file
 *  \brief Fills a cubic block of a scalarArray3d with a fixed value.
 *
 * Fills a cubic block of a scalarArray3d with a fixed value.
 * The position and dimension of the block as well as the fill-value are specified
 * in a (hopefully) self-explaining parameterfile.
 * Usage: fillBlockWithValue3D parameterfile
 *
...
int main ( int argc, char **argv ) {
  try {
    ...
    cerr << green << "\nLoading file '" << reset << fileName << green << "'...\n\n";
    ...
    cerr << green << "\nSaving to file '" << reset << fileName << green << "'...\n\n";
    inputImg.save ( fileName, qc::SaveTypeTrait<RealType>::BinarySaveType );
