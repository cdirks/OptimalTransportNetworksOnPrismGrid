...
/** \file
 *  \brief Saves a cutout from a quocmesh 3d data set in another scalarArray3d.
 *
 *  Saves a cutout from a quocmesh 3d data set in another scalarArray3d.
 *  The size and position (the latter as well in the source as in the destination array)
 *  can be determined in the corresponding (hopefully) self-explaining parameter-file.
 *  Usage: cutOutBlock3d parameterfile
 *
...
int main ( int argc, char** argv ) {
  try {
    ...
    if ( scaleTo0_1 ) {
      cerr << "scaling to [0,1]...";
      destImg.addToAll ( - destImg.getMinValue() );
    ...
    destImg.save ( imgName, qc::SaveTypeTrait<RealType>::BinarySaveType );
